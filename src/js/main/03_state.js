...
//   cache-format change, BUILD_ID not bumped.
// v5.30 (2026-09-22) — PACEMAKER DISCHARGE tweaks, per Kathryn: (1) the
//   "Submit claim only — not following" button now takes the purple/primary
//   fill (btn-p) when Pacemaker Discharge is active, instead of green — same
//   attention-grabbing weight the default "add to list" button normally has,
//   just handed to the other button. (2) the claim note (cb-notes) auto-fills
//   with "Asked to review patient for safety for discharge post PPM
//   insertion" when the preset turns on — rides on the claim exactly like a
//   hand-typed note, no new submit logic. Both in 07_consult.js only. No
//   backend change. No cache-format change, BUILD_ID not bumped.
// v5.31 (2026-09-24) — ONE-AT-A-TIME WRITE QUEUE. A rounding doctor who
//   sees several patients and then taps their daily-visit pills in a row
//   (EMMassie 24/09 10:09 PT: 7 pills in 14 s) fired 7+ saveClaims at once.
//   Every one queued server-side behind the Claims script lock, waits grew
//   to 15–34 s, the client's 20 s ceiling aborted them, and the retries of
//   saves that HAD landed (logged as edit_claim) piled on more load — 61
//   timeouts from one phone that morning, 3 lock refusals, zero data lost.
//   push() now sends sheet writes (saveClaim / savePatient / savePatients /
//   savePatientWithClaims / deleteClaim / deletePatient / saveGapNote /
//   mergePatientDemographics) ONE AT A TIME per device, in tap order. The
//   20 s timeout clock starts when a write is actually sent, not while it
//   waits its turn. Everything synchronous (claim gate capture, in-flight
//   guard, _pendingPush marking) still happens at tap time, unchanged.
//   Log/lookup calls (logChange, log*, addPhysician, saveRef…) are not
...
// ═══════════════════════════════════════════════════════════════════
// v5.31 — ONE-AT-A-TIME WRITE QUEUE (see version note above)
// FIFO promise chain. _writeQAcquire() resolves with a release function
// once every earlier queued write has released. depth = writes waiting or
// sending (read by the mandatory-update gate in 14_init.js).
// ═══════════════════════════════════════════════════════════════════
var WRITE_Q_ACTIONS_ = {
...
function _writeQAcquire() {
  ...
  q.depth++;
  var prev = q.tail, release;
  q.tail = new Promise(function(r){ release = r; });
  var _t0 = Date.now();
  return prev.then(function(){
    var released = false, valve = null;
    var rel = function(){
      if (released) return;
      released = true;
    ...
    if (waited > 1000) console.log('[writeQ] waited ' + waited + 'ms, ' + (q.depth - 1) + ' behind');
    return rel;
  });
}
...
async function push(action, body, wire) {
  ...
  setSyncState('syncing');
  // v5.31: wait for this device's earlier writes to finish before sending.
  var _qRel = null;
  if (WRITE_Q_ACTIONS_[action]) {
    _qRel = await _writeQAcquire();
    // Locked out as too old while we waited — same as the early guard.
    if (window._staleLocked) {
      _qRel(); _qRel = null;
