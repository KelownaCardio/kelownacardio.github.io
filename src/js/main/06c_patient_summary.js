...
function openPatientSummary(pid) {
  ...
  // All claims for this patient (Sheet + any already-loaded BQ archive),
  ...
  // Discharge summary line — "Discharged by (initials) on (date)".
  // dischargedBy is captured at discharge time going forward; discharges made
  // before this field existed show the date only.
  if (p.discharged) {
    var _dcDate = p.dischargeDate || '';
    if (!_dcDate && p.dischargedAt) {
      var _dcMs = parseDischargedAt(p.dischargedAt);
      if (_dcMs) _dcDate = fmtD(new Date(_dcMs));
    }
    var _dcBy = (p.dischargedBy != null) ? String(p.dischargedBy).trim() : '';
    if (_dcDate || _dcBy) {
      html += '<div style="font-size:11px;font-weight:700;color:var(--red-t);margin-top:7px">' +
              'Discharged ' + (_dcBy ? 'by ' + esc(_dcBy) + ' ' : '') +
              (_dcDate ? 'on ' + esc(_dcDate) : '') + '</div>';
    }
  }
  ...
  html +=   '<div id="cv-claim-count" style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px">' +
  ...
  // history reads "0 claims on record" while the data lives only in BQ.
  _augmentSummaryWithBQ(p);
}

// ── Merge Sheet claims with cached BigQuery archive claims for a patient ──
// _cvBQClaims holds normalized, display-only claims pulled from the warehouse
// for the currently-open patient. Sheet claims win on id collision (they may be
// newer / still editable). Used by the calendar, list, and gap calc so archived
// history appears everywhere the live claims do.
function _cvClaimsFor(p) {
  if (!p) return [];
  var local = (st.claims || []).filter(function(c) { return c.phn && p.phn && samePhn(c.phn, p.phn); });
  var have = {};
  local.forEach(function(c) { if (c.id != null) have[String(c.id)] = true; });
  var merged = local.slice();
  (window._cvBQClaims || []).forEach(function(c) {
    if (c && c.phn && p.phn && samePhn(c.phn, p.phn) && !have[String(c.id)]) merged.push(c);
  });
  merged.sort(function(a, b) { return parseDMY(a.date) - parseDMY(b.date); });
  return merged;
}

// Normalize one BigQuery claim row (getClaimsByPhn shape) into the app's claim
// shape. BQ uses feeCode/serviceDate/billingDoc/claimId; the query omits phn so
// we stamp the one we searched. Flagged _archived → rendered read-only (these
// are already submitted; edits/resubmits go through iClinic, not the app).
function _bqClaimToLocal(b, phn) {
  if (!b) return null;
  var date = b.serviceDate || '';
  if (!date && b.serviceDateParsed) {
    var m = String(b.serviceDateParsed).match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
    if (m) date = ('0' + m[3]).slice(-2) + '/' + ('0' + m[2]).slice(-2) + '/' + m[1];
  }
  if (!b.claimId || !date) return null;
  return {
    id:        b.claimId,
    alias:     b.billingDoc || '',
    fee:       String(b.feeCode || '').replace(/\.0+$/, ''),
    icd:       String(b.icd || '').replace(/\.0+$/, ''),
    units:     parseFloat(b.units) || 1,
    date:      date,
    loc:       b.loc || '',
    fac:       b.fac || '',
    ...
    endTime:   b.endTime || '',
    notes:     b.notes || '',
    refby:     String(b.refby || '').replace(/\.0+$/, ''),
    refbyName: b.refbyName || '',
    last:      b.last || '',
    first:     b.first || '',
    phn:       phn,
    dob:       b.dob || '',
    ...
    room:      b.room || '',
    source:    b.source || 'BQ',
    submitted: String(b.submitted) === 'true',
    _archived: true            // display-only — lives in BigQuery, not the Sheet
  };
}

// Async: fetch this patient's archived claims from BigQuery (action=claimHistory
// → getClaimsByPhn) and re-render the summary in place. Best-effort: silent on
// failure, and guarded so a slow response for a closed/!switched modal is dropped.
function _augmentSummaryWithBQ(p) {
  if (!p || !p.phn || !SHEETS_URL || !SHARED_KEY) return;
  var phnQ = String(p.phn).replace(/\D/g, '');
  if (phnQ.length !== 10) return;   // BQ getClaimsByPhn requires a 10-digit PHN
  var openedPid = p.id;
  _cvSetClaimCountLoading(true);
  fetch(SHEETS_URL + '?action=claimHistory&key=' + encodeURIComponent(SHARED_KEY) +
        '&phn=' + encodeURIComponent(phnQ) + '&_t=' + Date.now())
    .then(function(r) { return r.json(); })
    .then(function(j) {
      if (window._cvPid !== openedPid) return;   // modal closed or switched patient
      if (!j || j.ok === false || !Array.isArray(j.claims)) { _cvSetClaimCountLoading(false); return; }
      window._cvBQClaims = j.claims.map(function(b) { return _bqClaimToLocal(b, phnQ); })
                                   .filter(function(c) { return !!c; });
      _cvRerenderAfterBQ(p);
    })
    .catch(function() { if (window._cvPid === openedPid) _cvSetClaimCountLoading(false); });
}

// Toggle a small "loading archive…" suffix on the claim-count line.
function _cvSetClaimCountLoading(on) {
  var cc = document.getElementById('cv-claim-count');
  if (!cc) return;
  var base = cc.getAttribute('data-base') || cc.textContent.replace(/\s*·.*$/, '');
  cc.setAttribute('data-base', base);
  cc.innerHTML = on ? esc(base) + ' · loading archive…' : esc(base);
}

// Re-render calendar + list + discharge banner + claim count after BQ merge.
function _cvRerenderAfterBQ(p) {
  var claims = _cvClaimsFor(p);
  ...
  var arch = claims.filter(function(c) { return c._archived; }).length;
  var cc = document.getElementById('cv-claim-count');
  if (cc) {
    var txt = claims.length + ' claim' + (claims.length !== 1 ? 's' : '') + ' on record';
    cc.setAttribute('data-base', txt);
    cc.innerHTML = esc(txt) + (arch ? ' · ' + arch + ' from archive' : '');
  }
}
...
function _ptSummaryListHTML(p, claims) {
  ...
  dateOrder.forEach(function(date) {
    ...
    dayClaims.forEach(function(c) {
      ...
      var isPhone  = isPhoneAdviceClaim(c);   // PhoneAdvice web-form / phone consult
      var feeMeta  = FEES.find(function(f) { return f.code === c.fee; });
      // Phone consults get a dark-blue chip so they stand out from in-person
      // consults (yellow). CCU stays red; everything else keeps its catalogue clr.
      var feeChip  = isPhone ? 'chip-navy' : (isCCU ? 'chip-red' : (feeMeta && feeMeta.clr) || 'chip-grey');
      ...
      html +=     '</div>';
      if (c._archived) {
        // Submitted + archived in BigQuery — read-only in the app. Edits/resubmits
        // go through iClinic, not here.
        html +=   '<span class="chip chip-grey" style="font-size:8px;padding:1px 5px;white-space:nowrap" ' +
                  'title="Submitted and archived in BigQuery. Edit or resubmit via iClinic, not the app.">submitted</span>';
      } else {
...
var CV_TYPE_BG = {
  phone:'#5f75b0',                                            // softened dark blue (≈ rgba(27,58,143,.7) over white)
...
function _cvDayTypes(dayClaims) {
  ...
  var has = { phone:false, consult:false, ccu:false, daily:false, directive:false, combined:false, procedure:false };
  var hasAny = false;
  dayClaims.forEach(function(c) {
    hasAny = true;
    // Phone-advice (PhoneAdvice web-form / 10001) — checked first so it owns the
    // day colour (dark blue) rather than falling into the consult bucket.
    if (isPhoneAdviceClaim(c)) has.phone = true;
    // Consults (full / limited / emergency / prolonged counselling)
    else if (c.fee === '33010' || c.fee === '33012' || c.fee === '33005' || c.fee === '33014') has.consult = true;
  ...
  var out = ['phone','consult','ccu','daily','directive','combined','procedure'].filter(function(t) { return has[t]; });
...
function _ptSummaryCalendarHTML(p, claims) {
  ...
  html += '<div class="cv-legend">' +
            ...
            '<div class="cv-lg"><div class="cv-sw" style="background:rgba(27,58,143,.7)"></div>Phone</div>' +
  ...
  var rows = dayClaims.map(function(c) {
    ...
    return (
      '<div style="background:var(--surface2);border-radius:8px;padding:10px 12px;margin-bottom:8px">' +
        ...
        (c._archived
          ? '<div style="font-size:10px;color:var(--text3);margin-top:8px;font-style:italic">Submitted &amp; archived (BigQuery) — edit or resubmit via iClinic, not the app.</div>'
...
// Phone-advice (PhoneAdvice web-form) fee labels — these codes are written by
// the PhoneAdvice intake (web form / QuickChart email), not the in-app picker,
...
// claim source the web form / email pipeline stamps (WebForm / PhoneAdvice /
// QuickChart), or (c) an explicit phone claimType. Robust to new phone fee
// codes added on the web form, since source alone is enough.
function isPhoneAdviceClaim(c) {
  if (!c) return false;
  var fee = String(c.fee || '');
  if (PHONE_ADVICE_LABELS[fee]) return true;
  var src = String(c.source || '').toLowerCase();
  if (src === 'webform' || src === 'phoneadvice' || src === 'quickchart') return true;
  if (String(c.claimType || '').toLowerCase() === 'phone') return true;
  return false;
}
...
function getFeeLabel(fee) {
  ...
  if (f) return f.desc;
  // Phone-advice intake codes (not in the in-app picker catalogue)
  if (PHONE_ADVICE_LABELS[fee]) return PHONE_ADVICE_LABELS[fee];
