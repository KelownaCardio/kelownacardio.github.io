...
// v5.17 (2026-09-10): KGH stickers now print the DOB as "DD MMM YYYY"
// (e.g. "12 MAR 1948"); Meditech still prints "DD/MM/YYYY". The v5.09 parser
// only knew the numeric form, so a named-month date was "could not be read"
// and the DOB was blanked. ocrParseDobRaw() reads BOTH, still in code:
//   numeric      → day first (KGH is DD/MM, never MM/DD)
//   named month  → unambiguous; separators space / - . or none ("12MAR1948")
// Month words must be a real month (JAN..DEC, SEPT, full names) — no
// digit-for-letter guessing ("0CT" is refused, not corrected).
var _OCR_MON = { jan:1, feb:2, mar:3, apr:4, may:5, jun:6, jul:7, aug:8,
                 sep:9, sept:9, oct:10, nov:11, dec:12 };
var _OCR_MON_FULL = ['january','february','march','april','may','june','july',
                     'august','september','october','november','december'];
function ocrParseDobRaw(dobRaw) {
  var t = String(dobRaw || '').trim().replace(/\s+/g, ' ');
  var d, mo, y, m;
  if ((m = t.match(/^(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{4})$/))) {
    d = parseInt(m[1], 10); mo = parseInt(m[2], 10); y = parseInt(m[3], 10);
  } else if ((m = t.match(/^(\d{1,2})[\s\/\-.]*([A-Za-z]{3,9})\.?[\s\/\-.,]*(\d{4})$/))) {
    var w = m[2].toLowerCase();
    mo = _OCR_MON[w] || (_OCR_MON_FULL.indexOf(w) + 1) || 0;
    if (!mo) return { ok: false, why: '"' + m[2] + '" is not a month' };
    d = parseInt(m[1], 10); y = parseInt(m[3], 10);
  } else {
    return { ok: false, why: 'the date on the chart could not be read' };
  }
  ...
  return { ok: true, d: d, mo: mo, y: y,
           dmy: ('0' + d).slice(-2) + '/' + ('0' + mo).slice(-2) + '/' + y };
}

function ocrDobFromRaw(dobRaw, ageRaw) {
  var r = ocrParseDobRaw(dobRaw);
  if (!r.ok) return r;
  var d = r.d, mo = r.mo, y = r.y;
  ...
  // v5.17: first run of digits only — "78Y 3M" must read 78, not 783.
  var printed = (String(ageRaw == null ? '' : ageRaw).match(/\d+/) || [''])[0];
  if (!printed) return { ok: false, why: 'no age was printed to check it against' };
  printed = parseInt(printed, 10);
  if (printed !== age) {
    return { ok: false, why: 'the chart says age ' + printed + ' but ' + r.dmy + ' gives ' + age };
  }
...
var STICKER_PROMPT =
  ...
  '  dob           — date after "DOB". TWO printed formats exist:\n' +
  '                  • patient STICKER: "DD MMM YYYY", month as letters,\n' +
  '                    e.g. "12 MAR 1948" = 12th March 1948.\n' +
  '                  • Meditech screen: DD/MM/YYYY (day first, Canadian),\n' +
  '                    e.g. "03/09/1934" = 3rd September 1934.\n' +
  ...
  '                  character, INCLUDING any month letters, e.g.\n' +
  '                  "12 MAR 1948" or "06/05/1955". Do NOT reorder it, do\n' +
  '                  NOT convert it, do NOT correct it. Just copy what\n' +
  '                  you can see. "" if you cannot read it.\n' +
...
function handleOCRResult(data, bar) {
  ...
  if (_ocrAge) {
    ...
    // of refusing outright — still checked in code, never taken on trust.
    var _ocrSrc = _ocrParsed.ok ? _ocrRaw : fmtClaimDate(String(p.dob || '').trim());
    var _ocrChk = ocrDobFromRaw(_ocrSrc, _ocrAge);
    if (_ocrChk.ok) {
