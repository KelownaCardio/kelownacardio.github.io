...
// toggleClaimFollowUp (05_render.js) — turning it ON opens a mandatory-note
// modal, turning it OFF is a silent toggle. v5.29: GREEN styling
// (.cf-flag-btn) — deliberately distinct from the amber .foot-flag
// (handover) so the two flags read differently at a glance.
function _cfFlagBtn(c) {
  ...
  return '<button class="cf-flag-btn' + (on ? ' on' : '') + '" data-cid="' + esc(String(c.id||'')) + '"' +
    ...
    ' title="' + esc(on ? onTitle : 'Flag for KB Billing Follow Up') + '">' +
...
var FEES = [
  ...
  // v4.96: Emergency Bedside Care — per 30 min (or majority portion, ≥16 min)
  // of a life-threatening cardiac event. Start/end time + resuscitation note
  // mandatory (enforced in 06_claim_screen). Exported as literal '00081'
  // (leading zeros preserved — '81' is a different MSP item). If an initial
  // consult is billed for the same event, the consult covers the first 30 min
  // (Yes/No prompt at submit handles the split).
  { code:'00081',  desc:'Emergency bedside care — per 30 min of life-threatening cardiac event', amount:'$119.97', cat:'Emergency', clr:'chip-red'    },
